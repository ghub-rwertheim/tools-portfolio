...
start_index = 1
items_per_page = 100  #SCIM limit is 100 per page
all_users = []
...
try:
    while True:
        ...
        print(f"Retrieved {len(all_users)} of {total_results} users...")

        # Stop if we've collected all available users
        if len(all_users) >= total_results:
            break
